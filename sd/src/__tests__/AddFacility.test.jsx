...
jest.mock("@react-google-maps/api", () => ({
  ...
  GoogleMap: () => <section>Google Map</section>,
...
describe("AddFacility Component", () => {
  ...
  test("renders the form with all fields", () => {
    ...
    expect(
      screen.getByPlaceholderText("Search facility location")
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText("Subfacilities (Courts/Fields)")
    ).toBeInTheDocument();
  ...
  test("handles subfacility addition", async () => {
    ...
    fireEvent.change(
      screen.getByPlaceholderText("Capacity for this subfacility"),
      {
        target: { value: "10" },
      }
    );
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith(
        expect.stringContaining(
          "Cannot add subfacility. Remaining capacity is only 50"
        )
      );
    });
  });
  ...
  test("handles subfacility removal", async () => {
    ...
    expect(
      screen.queryByText("Court 1 (Capacity: 10)")
  ...
  test("displays loading state during submission", async () => {
    ...
    require("firebase/firestore").addDoc.mockImplementationOnce(
      () =>
        new Promise((resolve) =>
          setTimeout(() => resolve({ id: "test-id" }), 1000)
        )
    );
