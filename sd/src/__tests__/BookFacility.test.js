...
jest.mock("@react-google-maps/api", () => ({
  ...
  GoogleMap: ({ children }) => (
    <section data-testid="google-map">{children}</section>
  ),
  Marker: () => <section data-testid="marker"></section>,
}));
...
jest.mock("react-toastify", () => ({
  ...
  ToastContainer: () => <section data-testid="toast-container" />,
}));
...
describe("BookFacility Component", () => {
  ...
  beforeEach(() => {
    ...
    mockUseLocation.mockReturnValue({ state: {} }); // eslint-disable-next-line no-import-assign
    // @ts-ignore
    ...
    mockGetDocs.mockImplementation(async (ref) => {
      ...
      if (ref.path === "facilities") {
        return createMockQuerySnapshot(
          sampleFacilities.map((f) => createMockDoc(f, f.id))
        );
      }
      if (ref.path === "facilities/facility1/subfacilities") {
        return createMockQuerySnapshot(
          sampleSubfacilitiesFacility1.map((sf) => createMockDoc(sf, sf.id))
        );
      }
      if (ref.path.endsWith("/subfacilities")) {
        // General case for subfacilities
    ...
    mockGetDoc.mockImplementation(async (docRef) => {
      ...
      if (
        pathParts[0] === "facilities" &&
        pathParts[2] === "subfacilities" &&
        pathParts.length === 4
      ) {
        // e.g., facilities/facility1/subfacilities/sub1
        const facilityId = pathParts[1];
        const subfacilityId = pathParts[3];
        if (facilityId === "facility1") {
          const subfacility = sampleSubfacilitiesFacility1.find(
            (sf) => sf.id === subfacilityId
          );
    ...
    mockArrayUnion.mockImplementation((data) => ({
      _mockedArrayUnionItem: data,
    })); // Make it a bit more identifiable
  });
  ...
  test("renders and fetches facilities", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("option", { name: "Main Hall (Capacity: 100)" })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: "Sports Field (Capacity: 50)" })
      ).toBeInTheDocument();
    });

    expect(mockGetDocs).toHaveBeenCalledWith(
      expect.objectContaining({ path: "facilities" })
    );
  });
  ...
  test("selects a facility, calls onFacilitySelect, and shows subfacilities", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByRole("option", { name: "Court A (Capacity: 10)" })
      ).toBeInTheDocument();
    ...
    expect(mockGetDocs).toHaveBeenCalledWith(
      expect.objectContaining({ path: "facilities/facility1/subfacilities" })
    );
  });
  ...
  test("handles facility selection with no subfacilities", async () => {
    ...
    mockGetDocs.mockImplementation(async (ref) => {
      ...
      if (ref.path === "facilities") {
        return createMockQuerySnapshot([
          createMockDoc(facility3Data, facility3Data.id),
        ]);
      }
    ...
    mockGetDoc.mockImplementation(async (docRef) => {
      // Also ensure getDoc returns this facility's data
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("option", { name: "Meeting Room (Capacity: 20)" })
      ).toBeInTheDocument()
    );
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByLabelText("Select Court/Field")
      ).not.toBeInTheDocument();
    });
    expect(mockGetDocs).toHaveBeenCalledWith(
      expect.objectContaining({ path: "facilities/facility3/subfacilities" })
    );
  });
  ...
  test('shows "fully booked" message if no times are available', async () => {
    ...
    mockGetDocs.mockImplementation(async (ref) => {
      ...
      if (ref.path === "facilities") {
        ...
        return createMockQuerySnapshot([
          createMockDoc(bookedFacilityData, bookedFacilityData.id),
          ...sampleFacilities.slice(1).map((f) => createMockDoc(f, f.id)),
        ]);
      }
    ...
    await waitFor(() =>
      screen.getByRole("option", {
        name: `${bookedFacilityData.name} (Capacity: ${bookedFacilityData.capacity})`,
      })
    );
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByRole("button", { name: /13:00/ })
      ).not.toBeInTheDocument();
    });
  });
    ...
    await waitFor(() => {
      ...
      expect(mockUpdateDoc).toHaveBeenCalledWith(
        expect.objectContaining({ path: "facilities/facility2" }),
        {
          bookings: expect.objectContaining({
            _mockedArrayUnionItem: expect.anything(),
          }),
        }
      );
      expect(mockUpdateDoc).toHaveBeenCalledWith(
        expect.objectContaining({ path: `users/${sampleUser.uid}` }),
        {
          notifications: expect.objectContaining({
            _mockedArrayUnionItem: expect.objectContaining({
              message: expect.stringContaining("Sports Field"),
            }),
          }),
        }
      );
      expect(toast.success).toHaveBeenCalledWith(
        expect.stringContaining(
          `Booking request submitted for Sports Field on ${dateString} at 13:00 for 5 people.`
        )
      );
      expect(
        screen.queryByRole("button", { name: "13:00" })
  ...
  test("attendee input defaults to 1, updates, and does not show 'at least 1' warning for 0 input", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByText("Number of attendees must be at least 1")
      ).not.toBeInTheDocument();
    });
  });
  ...
  test("displays capacity warning when attendees exceed facility capacity", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("This facility only accommodates 50 people")
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("This facility only accommodates 50 people")
      ).not.toBeInTheDocument();
    });
  });
  ...
  test("displays capacity warning when attendees exceed subfacility capacity", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Select Court/Field"), {
      target: { value: "sub1" },
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText("This court/field only accommodates 10 people")
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("This court/field only accommodates 10 people")
      ).not.toBeInTheDocument();
    });
  });
  ...
  test("displays capacity warning when attendees exceed facility capacity (no subfacility selected)", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Select Facility"), {
      target: { value: "facility2" },
    }); // Sports Field, capacity 50

    await waitFor(() => {
      expect(screen.getByLabelText("Number of Attendees")).toHaveAttribute(
        "max",
        "50"
      );
    });
  ...
  test("date change resets selected time and fetches new available times", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "13:00" }));
    await waitFor(() =>
      expect(screen.getByRole("button", { name: "13:00" })).toHaveClass(
        "selected"
      )
    );
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByRole("button", { name: /13:00.*selected/ })
      ).not.toBeInTheDocument(); // Check it's not selected
      expect(screen.getByRole("button", { name: "13:00" })).not.toHaveClass(
        "selected"
      );
    ...
    expect(mockGetDoc).toHaveBeenCalledWith(
      expect.objectContaining({
        path: "facilities/facility1/subfacilities/sub1",
      })
    );
    await waitFor(() => {
  ...
  test("displays message to select date if facility is selected but date is not", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("Please select a date to see available times.")
      ).toBeInTheDocument();
    });
  });
  ...
  test("filters available times based on events blocking a facility (no subfacility selected)", async () => {
    ...
    mockGetDocs.mockImplementation(async (ref) => {
      ...
      if (ref.path === "facilities/facility2/subfacilities") {
        // facility2 has no subfacilities
        return createMockQuerySnapshot([]);
      }
      if (ref.path === "events") {
        return createMockQuerySnapshot([
          createMockDoc(eventBlockingFacility, eventBlockingFacility.id),
        ]);
      }
      return createMockQuerySnapshot([]);
    });
    ...
    mockGetDoc.mockImplementation(async (docRef) => {
      ...
      if (docRef.path === "facilities/facility2") {
        ...
        return createMockDoc(
          facility ? { ...facility, bookings: [] } : { bookings: [] },
          "facility2"
        );
      }
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole("button", { name: "13:00" })).toBeInTheDocument();
      expect(
        screen.queryByRole("button", { name: "14:00" })
      ).not.toBeInTheDocument();
      expect(
  ...
  test("filters available times based on events blocking a specific subfacility", async () => {
    ...
    mockGetDocs.mockImplementation(async (ref) => {
      ...
      if (ref.path === "events") {
        return createMockQuerySnapshot([
          createMockDoc(eventBlockingSubfacility, eventBlockingSubfacility.id),
        ]);
      }
      return createMockQuerySnapshot([]);
    });
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByRole("button", { name: "16:00" })
      ).not.toBeInTheDocument();
      expect(screen.getByRole("button", { name: "17:00" })).toBeInTheDocument();
    });
  ...
  test("initializes with facilityId from location state", async () => {
    ...
    mockUseLocation.mockReturnValue({
      state: { facilityId: initialFacilityId },
    });
    ...
    await waitFor(() => {
      ...
      expect(screen.getByLabelText("Select Facility")).toHaveValue(
        initialFacilityId
      );
  ...
  test("shows 'FULLY BOOKED' in subfacility dropdown if subfacility has all 5 time slots booked with 'approved' status for a selected date", async () => {
    ...
    const fullyBookedSubfacilityData = {
      ...
      bookings: [
        // All 5 slots booked and approved for this date
    ...
    mockGetDocs.mockImplementation(async (ref) => {
      ...
      if (ref.path === "facilities/facility1/subfacilities") {
        return createMockQuerySnapshot([
          createMockDoc(
            fullyBookedSubfacilityData,
            fullyBookedSubfacilityData.id
          ),
    ...
    mockGetDoc.mockImplementation(async (docRef) => {
      ...
      if (
        docRef.path ===
        `facilities/facility1/subfacilities/${fullyBookedSubfacilityData.id}`
      ) {
        return createMockDoc(
          fullyBookedSubfacilityData,
          fullyBookedSubfacilityData.id
      ...
      if (
        docRef.path ===
        `facilities/facility1/subfacilities/${otherSubfacilityData.id}`
      ) {
    ...
    fireEvent.change(screen.getByLabelText("Select Date"), {
      target: { value: dateWithFullSubfacility },
    });
    ...
    await waitFor(() => {
      ...
      const fullyBookedOption = screen.getByRole("option", {
        name: `${fullyBookedSubfacilityData.name} (Capacity: ${fullyBookedSubfacilityData.capacity}) - FULLY BOOKED`,
      });
      ...
      const otherOption = screen.getByRole("option", {
        name: `${otherSubfacilityData.name} (Capacity: ${otherSubfacilityData.capacity})`,
      });
  ...
  test("changing date resets selected time and fetches new available times", async () => {
    ...
    expect(mockGetDoc).toHaveBeenCalledWith(
      expect.objectContaining({ path: "facilities/facility1" })
    );
  });
});
