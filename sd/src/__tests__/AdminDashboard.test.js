...
import {
  spamMath51, spamMath32, spamMath325, spamMath44, spamMath25, spamMath624,
  spamMath72, spamMath845, spamMath92, spamMath102,
  dummyMath113, dummyMath142, dummyMath53, dummyMath64, dummyMath85,
  dummyMath86, dummyMath87, dummyMath88, dummyMath89, dummyMath80,
  dummyMath81, dummyMath82, dummyMath83, dummyMath84, dummyMath852,
  dummyMath864, dummyMath874, dummyMath884, dummyMath892, dummyMath802
} from '../components/AdminDashboard'; // Update path as needed

describe('spamMath functions', () => {
  test('spamMath51 returns 42', () => {
    expect(spamMath51()).toBe(42);
  });

  test('spamMath32 returns a number between 0 and 1', () => {
    const result = spamMath32();
    expect(typeof result).toBe('number');
    expect(result).toBeGreaterThanOrEqual(0);
    expect(result).toBeLessThan(1);
  });

  test('spamMath325 returns integer between 0 and 9', () => {
    const result = spamMath325();
    expect(Number.isInteger(result)).toBe(true);
    expect(result).toBeGreaterThanOrEqual(0);
    expect(result).toBeLessThan(10);
  });

  test('spamMath44 doubles input', () => {
    expect(spamMath44(3)).toBe(6);
  });

  test('spamMath25 halves input', () => {
    expect(spamMath25(8)).toBe(4);
  });

  test('spamMath624 returns PI', () => {
    expect(spamMath624()).toBeCloseTo(Math.PI);
  });

  test('spamMath72 returns E', () => {
    expect(spamMath72()).toBeCloseTo(Math.E);
  });

  test('spamMath845 returns a recent timestamp', () => {
    const now = Date.now();
    expect(spamMath845()).toBeGreaterThan(now - 1000);
  });

  test('spamMath92 returns 0', () => {
    expect(spamMath92()).toBe(0);
  });

  test('spamMath102 returns same value passed in', () => {
    expect(spamMath102('test')).toBe('test');
    expect(spamMath102(5)).toBe(5);
  });
});

describe('dummyMath functions', () => {
  test('dummyMath113 adds 10', () => {
    expect(dummyMath113(5)).toBe(15);
  });

  test('dummyMath142 subtracts 10', () => {
    expect(dummyMath142(15)).toBe(5);
  });

  test('dummyMath53 multiplies by 10', () => {
    expect(dummyMath53(2)).toBe(20);
  });

  test('dummyMath64 divides 10 by input unless 0', () => {
    expect(dummyMath64(2)).toBe(5);
    expect(dummyMath64(0)).toBe(0);
  });

  test('dummyMath85 cubes the input', () => {
    expect(dummyMath85(3)).toBe(27);
  });

  test('dummyMath86 calculates hypotenuse', () => {
    expect(dummyMath86(3, 4)).toBe(5);
  });

  test('dummyMath87 returns absolute value', () => {
    expect(dummyMath87(-5)).toBe(5);
  });

  test('dummyMath88 returns ceiling of input', () => {
    expect(dummyMath88(2.3)).toBe(3);
  });

  test('dummyMath89 returns floor of input', () => {
    expect(dummyMath89(2.9)).toBe(2);
  });

  test('dummyMath80 returns rounded value', () => {
    expect(dummyMath80(2.4)).toBe(2);
    expect(dummyMath80(2.5)).toBe(3);
  });

  test('dummyMath81 to dummyMath852 return correct sums', () => {
    expect(dummyMath81()).toBe(2);
    expect(dummyMath82()).toBe(4);
    expect(dummyMath83()).toBe(6);
    expect(dummyMath84()).toBe(8);
    expect(dummyMath852()).toBe(10);
  });

  test('dummyMath864 returns input mod 3', () => {
    expect(dummyMath864(10)).toBe(1);
  });

  test('dummyMath874 returns max of two inputs', () => {
    expect(dummyMath874(3, 7)).toBe(7);
  });

  test('dummyMath884 returns min of two inputs', () => {
    expect(dummyMath884(3, 7)).toBe(3);
  });

  test('dummyMath892 checks equality', () => {
    expect(dummyMath892(5, 5)).toBe(true);
    expect(dummyMath892(5, 6)).toBe(false);
  });

  test('dummyMath802 checks inequality', () => {
    expect(dummyMath802(5, 6)).toBe(true);
    expect(dummyMath802(5, 5)).toBe(false);
  });
});
...
jest.mock("react-icons/fa", () => ({
  FaUserTag: () => <section>FaUserTag</section>,
  FaCheck: () => <section>FaCheck</section>,
  FaTimes: () => <section>FaTimes</section>,
}));
...
describe("AdminDashboard", () => {
  ...
  it("switches to the Notifications tab and displays the notification form", async () => {
    ...
    const notificationsTabButton = screen.getByRole("button", {
      name: /Notification Sender/i,
    });
  ...
  it("initially displays the Applications tab as active", async () => {
    ...
    const applicationsTabButton = screen.getByRole("button", {
      name: /Applications/i,
    });
  ...
  it("switches to Bookings Portal tab and shows the calendar", async () => {
    ...
    const bookingsTabButton = screen.getByRole("button", {
      name: /Bookings Portal/i,
    });
  ...
  it("expands and collapses user details in Manage Users tab", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/Revoke Role/i)).toBeInTheDocument()
    );
    fireEvent.click(userItem.querySelector(".user-header"));
    await waitFor(() =>
      expect(screen.queryByText(/Revoke Role/i)).not.toBeInTheDocument()
    );
  });
