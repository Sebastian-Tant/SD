...
jest.mock('../firebase', () => ({ // Adjust path to your firebase config
  db: mockDb,
}));

jest.mock('firebase/firestore', () => ({
  collection: (db, path, ...pathSegments) => mockCollectionImpl(db, path, ...pathSegments),
...
jest.mock('framer-motion', () => {
  ...
  return {
    ...
    motion: { // Simplify motion components for testing
...
jest.mock('@fortawesome/react-fontawesome', () => ({
  FontAwesomeIcon: jest.fn(({ icon }) => <i data-testid={`fa-icon-${icon.iconName}`} />),
}));
    ...
    mockGetDocsImpl.mockImplementation(async (queryRef) => {
      if (queryRef.path === 'facilities') {
        return Promise.resolve({
          docs: [
            { id: 'facility1', data: () => ({ name: 'Gym', has_subfacilities: false }) },
            { id: 'facility2', data: () => ({ name: 'Pool', has_subfacilities: true }) },
            { id: 'facility3', data: () => ({ name: 'Yoga Studio', has_subfacilities: false }) },
          ],
        });
      }
...
/*{test('renders loading state initially and then displays chart title and default facility', async () => {
  ...
  test('fetches bookings for "All Facilities" and processes them', async () => {
    ...
    mockGetDocImpl.mockImplementation(async (docRef) => { // For facilities without subfacilities
      if (docRef.path === 'facilities/facility1') { // Gym
        return { exists: () => true, data: () => ({ name: 'Gym', bookings: [{ time: '13:00' }, { time: '14:00' }] }) };
      }
      return { exists: () => true, data: () => ({ bookings: [] }) };
    });
    mockGetDocsImpl.mockImplementation(async (queryRef) => { // For facilities list and subfacilities
      if (queryRef.path === 'facilities') {
        return {
          docs: [
            { id: 'facility1', data: () => ({ name: 'Gym', has_subfacilities: false }) },
      ...
      if (queryRef.path === 'facilities/facility2/subfacilities') { // Pool's subfacilities
        return {
          docs: [
            { id: 'sub1', data: () => ({ name: 'Lane 1', bookings: [{ time: '13:00' }, { time: '15:00' }] }) },
          ],
        };
      }
      return { docs: [] };
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('mock-line-chart')).toHaveTextContent('ChartData: [2,1,1,0,0]');
      expect(screen.getByTestId('mock-line-chart')).toHaveTextContent('YAxisLabel: Average Bookings');
    });
...
test('dropdown functionality: opens, closes, and selects a facility', async () => {
  ...
  const dropdownButton = screen.getByRole('button', { name: /All Facilities/i });
  ...
  await waitFor(() => {
    ...
    expect(screen.getByText('Afternoon bookings for Gym')).toBeInTheDocument();
  });
});
...
test('displays specific facility data when selected (facility without subfacilities)', async () => {
  ...
  await waitFor(() => {
    ...
    expect(screen.getByTestId('mock-line-chart')).toHaveTextContent('ChartData: [0,0,2,1,0]');
...
test('displays specific facility data when selected (facility with subfacilities)', async () => {
  ...
  await waitFor(() => {
    ...
    expect(screen.getByTestId('mock-line-chart')).toHaveTextContent('ChartData: [0,2,0,0,0]');
    expect(screen.getByTestId('mock-line-chart')).toHaveTextContent('YAxisLabel: Total Bookings');
  });
});
...
/*test('handles empty or invalid booking times defaulting to "5 PM"', async () => {
  ...
  test('displays "No booking data available" recommendation when no data', async () => {
    ...
    mockGetDocImpl.mockResolvedValue({ exists: () => true, data: () => ({ bookings: [] }) });
    mockGetDocsImpl.mockImplementation(async (queryRef) => {
      if (queryRef.path === 'facilities') {
        return {
          docs: [{ id: 'facility1', data: () => ({ name: 'Gym', has_subfacilities: false }) }],
        };
      }
    ...
    render(<PeakHoursChart />);
    await waitFor(() => {
      expect(screen.getByTestId('mock-line-chart')).toHaveTextContent('ChartData: [0,0,0,0,0]');
    });
    expect(screen.getByText('No booking data available')).toBeInTheDocument();
    // Busiest/Quietest times will be empty, values will be 0
    const busiestValueElements = screen.getAllByText(/0 avg\. bookings|0 total bookings/);
...
const setupAndRenderGymWithBookings = async (gymBookings) => {
  mockGetDocImpl.mockImplementation(async (docRef) => { // For Gym
    if (docRef.path === 'facilities/facility1') {
      return { exists: () => true, data: () => ({ name: 'Gym', bookings: gymBookings }) };
    }
  ...
  render(<PeakHoursChart />);
  await waitFor(() => screen.getByRole('button', { name: /All Facilities/i })); // Wait for initial load
  ...
  await waitFor(() => screen.getByRole('button', { name: /Gym/i })); // Wait for selection to apply
    ...
    test('recommends "Consider adjusting pricing..." when demand is skewed (maxValue/minValue > 3) and minValue > 0', async () => {
      ...
      const bookings = [
        { time: '13:00' }, { time: '13:00' }, { time: '13:00' }, { time: '13:00' }, // 4 at 1 PM
        { time: '14:00' }, { time: '15:00' }, { time: '16:00' }, { time: '17:00' }  // 1 each for other slots
      ];
      await setupAndRenderGymWithBookings(bookings);
      await waitFor(() => {
        expect(screen.getByText('Consider adjusting pricing to balance demand')).toBeInTheDocument();
      });
    });
    ...
    test('recommends "Demand is well distributed" otherwise (ratio <= 3 and minValue > 0)', async () => {
      ...
      const bookings = [
        { time: '13:00' }, { time: '13:00' }, // 2 at 1 PM
        { time: '14:00' }, { time: '15:00' }, { time: '16:00' }, { time: '17:00' } // 1 each for other slots
      ];
      await setupAndRenderGymWithBookings(bookings);
      await waitFor(() => {
        expect(screen.getByText('Demand is well distributed')).toBeInTheDocument();
      });
    });
  });
});