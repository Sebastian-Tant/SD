...
// Simple Sun/Moon icons for the toggle button (you can replace with SVGs or an icon library)
    ...
    <main className="main-content"> {/* BG and text color handled by CSS variables on body/html */}
      ...
      <div className={`cards-grid mb-12 ${cardsVisible ? 'cards-visible' : ''}`}>
        <ReportCard 
          ...
          trendColorClass={isLoading ? "" : (stats.bookingTrend >= 0 ? "trend-positive" : "trend-negative")}
          iconType="calendar-check"
          trendIcon={isLoading ? null : (stats.bookingTrend >= 0 ? 'arrow-up' : 'arrow-down')}
          className="report-card animated-gradient-indigo animate-fadeInUp delay-100"
        />
        <ReportCard 
          ...
          trendColorClass={isLoading ? "" : "trend-warning"}
          iconType="tools"
